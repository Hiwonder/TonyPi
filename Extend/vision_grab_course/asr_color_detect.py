...
cmd_dict = {b"\xaa\x55\x03\x00\xfb": 'wakeup',
            ...
            b"\xaa\x55\x00\x08\xfb": 'red',
            b"\xaa\x55\x00\x09\xfb": 'green',
            b"\xaa\x55\x00\x0A\xfb": 'blue'}
...
def asr_detect():
    ...
    while True:
        if not state:
            ...
            if data != b'':
                if data in cmd_dict:
                    print("result:", cmd_dict[data])
                    if cmd_dict[data] == 'wakeup':
                        speech.play_audio(os.path.join(audio_path, 'wakeup.wav'))
                    elif cmd_dict[data] == 'sleep':
                        speech.play_audio(os.path.join(audio_path, 'dong.wav')) 
                    elif cmd_dict[data] == 'red':
                        target_color = 'red'
                        state = True
                    elif cmd_dict[data] == 'green':
                        target_color = 'green'
                        state = True
                    elif cmd_dict[data] == 'blue':
                        target_color = 'blue'
                        state = True
                    else:
                        target_color = 'None'
...
# 运行子线程(run sub-thread)
th = threading.Thread(target=asr_detect)
th.daemon = True
th.start()
